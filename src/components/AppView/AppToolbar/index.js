...
const AppToolbar = function(props) {
  ...
  const handleClick = (evt) => {
    ...
    if (flag !== 'montecarlo' || allowMonteCarlo) {
      props.handleClick(evt, flag)
    }
  }
  const showCharts = props.showCharts
  const allowMonteCarlo = props.allowMonteCarlo
  ...
  function onVisibleChange(visible) {
    props.handleClick(null, 'chartpicker') //alert Forecaster of possible chart hash url
    // debugger
  }
